...
def get_client_stream_requests():
    while True:
        name = input("Please enter a name (or Tschuss to stop chatting): ")

        if name == "Tschuss":
            break

        hello_request = greet_pb2.HelloRequest(greeting="Servus", name=name)
        yield hello_request
        time.sleep(1)
...
def run():
    with grpc.insecure_channel('localhost:50051') as channel:
        ...
        if rpc_call == "1":
            hello_request = greet_pb2.HelloRequest(greeting="Servus", name="YouTube")
